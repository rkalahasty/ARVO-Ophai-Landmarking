from .keras_unet_collection.Utility import *
from .keras_unet_collection.Import import *
...
def HBA_skip(
    ...
    nn = HBA(num_heads=heads, bottleneck_dimension=target_dimension, name=name + "_mhsa")
    nn = batchnorm_with_activation(nn, activation=None, zero_gamma=True, name=name + "_3_")
    nn = UpSampling2D(size=strides)(nn)

    return nn

def hbaunet(input_shape, dropout_rate = 0.4, use_attnDecoder = False, skip = False, num_layers = 3):
    global attn_3, attn_2, attn_1
    resnet_base = Encoder(
      include_top=True,
      classes = 2,
